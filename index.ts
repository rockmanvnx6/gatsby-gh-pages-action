...
async function run(): Promise<void> {
    try {
        ...
        const isSameRepo =
            !deployRepo || deployRepo === github.context.repo.repo;

        if (isSameRepo && github.context.ref === `refs/heads/${deployBranch}`) {
            console.log(
                `Triggered by branch used to deploy: ${github.context.ref}.`
            );
        ...
        const skipPublish = (
            core.getInput("skip-publish") || "false"
        ).toUpperCase();
        if (skipPublish === "TRUE") {
        ...
        const gitUserName =
            core.getInput("git-config-name") || github.context.actor;
        const gitEmail =
